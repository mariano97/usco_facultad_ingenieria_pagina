...
export default class RedesPrograma extends mixins(JhiDataUtils) {
  ...
  public redesProgramas: IRedesPrograma[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllRedesProgramas();
  }
  ...
  public retrieveAllRedesProgramas(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.redesProgramas.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IRedesPrograma): void {
  ...
  public removeRedesPrograma(): void {
    this.redesProgramaService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('paginaFacultadIngenieriaProyectoApp.redesPrograma.deleted', { param: this.removeId });
