...
export default class Programa extends mixins(JhiDataUtils) {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.programas.push(res.data[i]);
            }
  ...
  public removePrograma(): void {
    this.programaService()
      ...
      .then(() => {
        const message = this.$t('paginaFacultadIngenieriaProyectoApp.programa.deleted', { param: this.removeId });
